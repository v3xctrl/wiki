...
> **NOTE:** Often times this is an issue with the SD card. Make sure you use a high quality SD card since problems will only accumulate from this point.
...
When you see the modem to connect, disconnect and re-enummerate in DMESG, it indicates a bad connection.

This might especially be a problem with the modem that comes with pogo pins. The best way to fix this is to remove the pogo pins and solder directly to the pads. This can be a bit finecky. Alternatively it can also help to tin the pads slightly.

## Reverse SSH shell
> Only use this when you know what you are doing or when any of team asks you to.

Sometimes it might be helpful to have access to the streamer via SSH when in field. For this a reverse SSH shell is in place. In the "Extras" tab you will find a setting for a remote Server which the reverse shell will open to.
...
You can then start the Reverse Shell view the viewers "Streamer" menu.

## SD card
SD card issues are more common then you might think and symptoms are unfortunately super random.
...
### Useful commands (NON-DESTRUCIVE)
```
