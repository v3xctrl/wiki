...
## Typical Output During Satellite Drop (issue under investigation)
...
If `ant=OK` and `jam` is low during a drop, the cause is likely signal obstruction or a brief module reset rather than RF interference or antenna failure.
