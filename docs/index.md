...
Follow the [Streamer installation guide](Streamer.md).
...
!!! info "Prerequisites"
    * Completed [step 1](#1-rpi-setup) and [step 2](#2-viewer-setup)
    * The viewer is running
    * You know the internal IP address of your viewer machine

While keeping the viewer running, open the streamers web interface: `http://v3xctrl.local`. In the server section make sure "_Connection Mode_" is set to "_direct_". In the Host field, set the internal IP address of your viewer machine which you have established in [step 2](#2-viewer-setup). Scroll to the bottom and click "_Save_".
...
!!! info "Prerequisites"
    * A compatible modem (see [Modems](Modems.md))
    * A prepared SIM card (see [SIM card documentation](SIM.md))
